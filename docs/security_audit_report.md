...
- **位置**: `internal/security/filter.go`
- **状态**: ✅ 已修复（在 Filter 构造阶段预编译正则并缓存，运行时不再动态编译，过滤逻辑改用编译结果以消除 ReDoS 风险）
- **修复摘要**:
  - 新增 `compiledPattern` 结构缓存正则与原始表达式，`NewFilter` 统一调用 `compilePatterns`，忽略无效正则并在运行时复用。
  - `Check` 中的黑名单、白名单与参数校验全部使用预编译对象，避免每次请求重新编译带来的性能与安全隐患。
  - 测试扩展 (`internal/security/filter_test.go`) 验证无效正则会被自动跳过，防止配置错误导致服务异常。
- **后续建议**:
  - 在配置加载阶段记录被忽略的无效正则，并在启动日志中提示运维修正。
  - 若未来支持用户动态下发规则，可引入编译耗时统计或限速，进一步降低复杂表达式风险。

### 6. 信息泄露（中危）

- **位置**: `internal/mcp/mcp_server.go`
- **状态**: ✅ 已修复（`handleTestConnection`/`handleListHosts` 仅返回最小化的状态信息，移除主机地址、用户名、密钥路径等敏感字段）
- **修复摘要**:
  - 连接测试的响应只保留 host_id、状态与时间戳，敏感信息一律留在服务端日志中，不再通过 API 返回。
  - 主机列表接口输出精简字段（id、连接策略等），并去掉私钥路径、known_hosts 等隐私数据，防止枚举内部结构。
  - 新逻辑保持原有工具调用能力，同时满足最小权限原则。
- **后续建议**:
  - 若需展示详细信息，可在受控界面（需鉴权）中单独查询；对外接口保持最小可用集。
  - 结合审计日志监控敏感接口的调用频率，及时发现异常访问。

### 7. 输入校验缺失（中危）

- **位置**: `internal/execsvc/service.go`
- **状态**: ✅ 已修复（`validateParameters` 实现参数必填校验、类型转换、正则验证，并在执行前阻断非法输入）
- **修复摘要**:
  - 支持 `string`、`integer`、`float`、`boolean` 四类参数的强类型检查，兼容字符串/数字输入并在失败时给出明确错误。
  - 自动应用正则验证 (`validation`)，配套单测覆盖缺参、类型不符、正则不匹配等场景，确保脚本执行前即报错。
  - 对成功通过校验的参数进行统一转换，保证模板渲染阶段获取的值稳定可靠。
- **后续建议**:
  - 结合业务需要扩展更多参数类型（如枚举、列表），并在配置文档中补充示例。
  - 在脚本配置入口提供静态校验工具，提前发现配置层面的错误。
...
- **状态**: ✅ 已修复（引入集中化审计日志管道并覆盖拒绝/异常场景）
- **修复摘要**:
  - 新增 `internal/audit` 包，定义统一的事件结构、上下文传播与多通道输出（JSON 文本、文件）。
  - `config.Config` 引入 `audit_logging` 配置段，支持开关、输出目的地及文件路径，默认启用独立审计日志文件。
  - `security.Filter`、`execsvc.Service` 与 `mcp.MCPServer` 注入审计记录器：拒绝规则、命令执行成功/失败、脚本校验、鉴权失败等关键路径均产生日志，包含请求 ID、操作者、规则编号等元信息。
  - MCP 层透传请求上下文（来源 IP、调用者标识、工具名称），确保下游模块输出的审计日志可串联完整链路。
- **验证**:
  - `go test ./...`（新增审计记录器、过滤器审计输出、鉴权失败等单元测试）。
