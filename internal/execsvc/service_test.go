...
func TestService_ExecuteScript(t *testing.T) {
	...
	if !strings.Contains(result.Stdout, "'Alice'") || !strings.Contains(result.Stdout, "'30'") {
...
func TestService_RenderTemplate(t *testing.T) {
	...
	result, err := svc.renderTemplate("echo Hello {{.name}}", map[string]interface{}{"name": "World"}, false)
	if err != nil {
...
func TestService_RenderTemplate_MissingParameter(t *testing.T) {
	...
	result, err := svc.renderTemplate("echo {{.name}} {{.age}}", map[string]interface{}{"name": "Alice"}, false)
	if err != nil {
		t.Fatalf("期望模板渲染成功，但得到错误: %v", err)
	}
	if result != "echo Alice <no value>" {
		t.Errorf("缺少参数时应输出 <no value>，得到 '%s'", result)
	}
}

func TestService_ExecuteScript_InvalidType(t *testing.T) {
	svc := newTestServiceWithConfig(t, func(cfg *config.Config) {
		cfg.Scripts = []config.ScriptConfig{
			{
				Name:         "type-check",
				Template:     "echo Count {{.count}}",
				Parameters:   []config.ScriptParameter{{Name: "count", Type: "integer", Required: true}},
				AllowedHosts: []string{"*"},
			},
		}
	})
	...
	_, err := svc.ExecuteScript(context.Background(), ScriptRequest{
		...
		Parameters: map[string]interface{}{"count": "abc"},
	})
	if err == nil || !strings.Contains(err.Error(), "类型错误") {
		t.Fatalf("期望类型校验失败，得到: %v", err)
	}
}

func TestService_ExecuteScript_RegexValidation(t *testing.T) {
	svc := newTestServiceWithConfig(t, func(cfg *config.Config) {
		cfg.Scripts = []config.ScriptConfig{
			{
				Name:         "regex-check",
				Template:     "echo ID {{.id}}",
				Parameters:   []config.ScriptParameter{{Name: "id", Type: "string", Required: true, Validation: "^id-[0-9]+$"}},
				AllowedHosts: []string{"*"},
			},
		}
	})
	...
	_, err := svc.ExecuteScript(context.Background(), ScriptRequest{
		HostID:     "test-host",
		ScriptName: "regex-check",
		Parameters: map[string]interface{}{"id": "invalid"},
	})
	if err == nil || !strings.Contains(err.Error(), "未通过验证") {
		t.Fatalf("期望正则校验失败，得到: %v", err)
	}
}
